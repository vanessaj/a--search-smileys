...
public class GridSearch extends JFrame
{
	private static final String TITLE="A* Algorithm";
	...
	public GridSearch()
	{
		...
		//Integer[] smileyHandlers = {0,6,42,48};
		...
		tf.setText("Here are the instructions");
		content.add(tf);*/
	...
	public static Stack<Integer> startMoving(int startPos){
		...
		// array for obstacles
		int[] obstacles = {3,8,11,15,18,22,23,25,30,37,38,39};
		...
		while(!openset.isEmpty()){
			...
			// CHANGE COLOUR OF WHERE ALGORITHM CURRENTLY LOOKING
			// **** couldn't get to repaint, would only repaint after all nodes were examined	
			   /* try {
					Thread.sleep(200);
					JButton btn = (JButton) cells[current];
					// change the button style
					btn.setBorderPainted(false);
					btn.setOpaque(true);
					// using black for obstacles
	...
	public static ArrayList<Integer> getNeighbours(int cell){
		...
		// remove obstacles from neighbour list, not possible to travel to them
		int[] obstacles = {8,15,22,23,30,37,38,39,3,11,18,25};
	...
	// 1st row is numbered 0,1,2,3,...,6 ; 2nd row is 7,8,...,13, ... ; 7th row is 42,43,...,48
